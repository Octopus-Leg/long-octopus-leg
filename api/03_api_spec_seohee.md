...
## 5️⃣ OAS를 스프링 코드로 변환
- **프로젝트 준비**
  - Spring Initializr를 통해 `Gradle`, `Java`, `Spring Boot` 기반의 프로젝트를 생성함.
...
- **코드 생성 7단계 프로세스**
    ...
    - `complieJava.dependsOn swaggerSources.eStore.code`, `processResources { dependsOn(generateSwaggerCode) }`
...
- **필요성**: 컨트롤러마다 `try-catch`로 예외를 잡으면 코드가 중복되고 지저분해짐. 중앙에서 일관된 형식의 에러 응답을 내려주기 위해 필요함.
- **구현 구성 요소**
  ...
  - **`RestApiErrorHandler` (@ControllerAdvice)**:
